...
function xmldb_predefinedlabels_upgrade($oldversion) {
    global $CFG, $DB;
    ...
    if ($oldversion < 2013021400) {
        // find all courses that contain predefinedlabelss and reset their cache
        $modid = $DB->get_field_sql("SELECT id FROM {modules} WHERE name=?",
                array('predefinedlabels'));
        if ($modid) {
        ...
        // predefinedlabels savepoint reached
        upgrade_mod_savepoint(true, 2013021400, 'predefinedlabels');
    }
